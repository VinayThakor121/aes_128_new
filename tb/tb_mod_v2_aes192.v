// Modified AES V2 – AES-192 Testbench
// Paper: "Modified Advanced Encryption Standard Algorithm for Information Security"
//        (O.C. Abikoye et al.)
//
// Test vectors: NIST FIPS-197 Appendix C.2 key and plaintext.
// Primary test: round-trip correctness (encrypt → decrypt → original plaintext).
// Also verifies ciphertext differs from standard AES-192 (modification is active).
`timescale 1ns / 1ps

module tb_mod_v2_aes192;

localparam KEY_SIZE = 192;
...
// Standard AES-192 expected ciphertext (to verify the modification is active)
...
initial begin
    $display("====================================================");
    $display("  Modified AES V2 – 192-bit Key Test");
    $display("====================================================");
    $display("  Key       : %h", KEY);
