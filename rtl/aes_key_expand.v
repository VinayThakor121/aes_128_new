// AES Key Expansion (Combinatorial, Fully Unrolled)
// FIPS-197 Section 5.2  (KeyExpansion)
//
// Supports KEY_SIZE = 128 (Nk=4, Nr=10, 44 words),
//           KEY_SIZE = 192 (Nk=6, Nr=12, 52 words),
//           KEY_SIZE = 256 (Nk=8, Nr=14, 60 words).
//
// Output: round_keys_flat[1919:0]
//   word[i] occupies bits [1919 - 32*i  -: 32]
//   Round key r = {word[4r], word[4r+1], word[4r+2], word[4r+3]}
//              = round_keys_flat[1919 - 128*r  -: 128]
//   Words beyond 4*(Nr+1) are driven to zero and unused.
`timescale 1ns / 1ps
...
module aes_key_expand #(
    ...
    output [1919:0]       round_keys_flat
);

localparam Nk    = (KEY_SIZE == 128) ? 4 : (KEY_SIZE == 192) ? 6 : 8;
localparam Nr    = (KEY_SIZE == 128) ? 10 : (KEY_SIZE == 192) ? 12 : 14;
localparam TOTAL = 4 * (Nr + 1);   // 44 / 52 / 60

// ----------------------------------------------------------------
// Internal S-box (forward) implemented as function to avoid
// needing a module port from a combinatorial generate block.
// ----------------------------------------------------------------
function [7:0] sbox_f;
    input [7:0] b;
    case (b)
...
// RotWord: rotate word left by 8 bits  [a,b,c,d] -> [b,c,d,a]
function [31:0] rot_word;
    input [31:0] w;
    rot_word = {w[23:0], w[31:24]};
endfunction

// Rcon[i] = {x^(i-1) in GF(2^8), 0x00, 0x00, 0x00}  (i >= 1)
function [31:0] rcon;
    input integer idx;
    reg [7:0] rc;
    begin
...
always @(*) begin : key_sched
    ...
    for (i = 0; i < Nk; i = i + 1)
        w[i] = key_in[(KEY_SIZE-1) - 32*i -: 32];

    // Expand to TOTAL words
    for (i = Nk; i < TOTAL; i = i + 1) begin
        if ((i % Nk) == 0)
            w[i] = w[i-Nk] ^ sub_word(rot_word(w[i-1])) ^ rcon(i/Nk);
        else if ((Nk == 8) && ((i % Nk) == 4))
    ...
    // Pack into flat output (word[0] at MSB)
    for (i = 0; i < 60; i = i + 1)
        rk_flat[1919 - 32*i -: 32] = w[i];
end

assign round_keys_flat = rk_flat;
