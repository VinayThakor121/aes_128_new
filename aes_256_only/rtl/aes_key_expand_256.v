`timescale 1ns / 1ps
//
// aes_key_expand_256 — Combinatorial AES-256 key schedule
//
// FIPS-197 Section 5.2  (KeyExpansion, Nk=8, Nr=14, TOTAL=60 words)
//
// AES-256 specific: at positions where i%8==4, SubWord(w[i-1]) is applied
// WITHOUT RotWord and WITHOUT Rcon (extra SubWord step unique to AES-256).
//
// Output: round_keys_flat[1919:0]
//   word[i] occupies bits [1919 - 32*i -: 32]
//   Round key r = round_keys_flat[1919 - 128*r -: 128]   r = 0..14
//   Words beyond 4*15=60 are driven to zero and unused.
...
always @(*) begin : key_sched
    ...
    // Load initial 8 words from key_in
    for (i = 0; i < Nk; i = i + 1)
        w[i] = key_in[255 - 32*i -: 32];

    // Expand to TOTAL words
    for (i = Nk; i < TOTAL; i = i + 1) begin
        if ((i % Nk) == 0)
            w[i] = w[i-Nk] ^ sub_word(rot_word(w[i-1])) ^ rcon(i/Nk);
        else if ((i % Nk) == 4)   // AES-256 specific: extra SubWord at pos 4
