`timescale 1ns / 1ps
//
// aes_key_expand_256_seq — Sequential AES-256 key schedule
// Timing-optimised for Artix-7 at 100 MHz
//
// Motivation
// ----------
// A fully combinatorial AES-256 key schedule chains multiple SubWord calls
// through a long loop, producing a critical path well beyond 10 ns at 100 MHz.
// This module spreads the work over 14 clock cycles so that each cycle
// contains at most ONE SubWord call, keeping each stage ≈ 6–8.5 ns.
//
// AES-256 key schedule structure (Nk=8, Nr=14, TOTAL=60 words)
// -------------------------------------------------------------
// Standard FIPS-197 expansion:
//   i%8 == 0 : w[i] = w[i-8] ^ SubWord(RotWord(w[i-1])) ^ Rcon[i/8]
//   i%8 == 4 : w[i] = w[i-8] ^ SubWord(w[i-1])          (AES-256 specific)
//   else      : w[i] = w[i-8] ^ w[i-1]
//
// Phase assignment (one SubWord per phase, 4 words computed per phase):
// -----------------------------------------------------------------------
//   Phase 0  (key_load=1): latch key_in → w[0..7]
//   Phase 1  : compute w[ 8..11]  (SubWord(RotWord(w[ 7])) ^ rcon[1])
//   Phase 2  : compute w[12..15]  (SubWord(w[11]))
//   Phase 3  : compute w[16..19]  (SubWord(RotWord(w[15])) ^ rcon[2])
//   Phase 4  : compute w[20..23]  (SubWord(w[19]))
//   Phase 5  : compute w[24..27]  (SubWord(RotWord(w[23])) ^ rcon[3])
//   Phase 6  : compute w[28..31]  (SubWord(w[27]))
//   Phase 7  : compute w[32..35]  (SubWord(RotWord(w[31])) ^ rcon[4])
//   Phase 8  : compute w[36..39]  (SubWord(w[35]))
//   Phase 9  : compute w[40..43]  (SubWord(RotWord(w[39])) ^ rcon[5])
//   Phase 10 : compute w[44..47]  (SubWord(w[43]))
//   Phase 11 : compute w[48..51]  (SubWord(RotWord(w[47])) ^ rcon[6])
//   Phase 12 : compute w[52..55]  (SubWord(w[51]))
//   Phase 13 : compute w[56..59]  (SubWord(RotWord(w[55])) ^ rcon[7])
//              → valid = 1
//   Phase 14 : idle/done (valid stays high until next key_load)
//
// Critical path per phase
// -----------------------
//   Odd phases  (RotWord+SubWord): register → RotWord (wiring) → SubWord
//               (~5.5 ns) → XOR rcon → XOR word-tree (~1 ns) → register
//               Total ≈ 6.5 ns  ✓
//   Even phases (SubWord only):    register → SubWord (~5.5 ns)
//               → XOR word-tree (~1 ns) → register
//               Total ≈ 6.5 ns  ✓
//
// Output layout (identical to aes_key_expand_256)
// -----------------------------------------------
//   round_keys_flat[1919 - 32*i -: 32] = w[i]   for i = 0 .. 59

module aes_key_expand_256_seq (
    input  wire         clk,
    input  wire         rst,
    input  wire [255:0] key_in,
    input  wire         key_load,    // 1-cycle pulse to start expansion
    output wire [1919:0] round_keys_flat,
...
// -----------------------------------------------------------------------
// Word storage  (60 words = 15 × 128-bit round keys)
// -----------------------------------------------------------------------
reg [31:0] w [0:59];
reg [4:0]  phase;   // 1..13 active; 5'd14 = idle/done

// -----------------------------------------------------------------------
// Odd-phase SubWord wires: SubWord(RotWord(w[k])) ^ rcon[j]
// All read from REGISTERED w[] values.
// -----------------------------------------------------------------------
wire [31:0] sw_a1  = sub_word_f(rot_word_f(w[ 7])) ^ 32'h01000000;  // phase 1
wire [31:0] sw_a2  = sub_word_f(rot_word_f(w[15])) ^ 32'h02000000;  // phase 3
wire [31:0] sw_a3  = sub_word_f(rot_word_f(w[23])) ^ 32'h04000000;  // phase 5
wire [31:0] sw_a4  = sub_word_f(rot_word_f(w[31])) ^ 32'h08000000;  // phase 7
wire [31:0] sw_a5  = sub_word_f(rot_word_f(w[39])) ^ 32'h10000000;  // phase 9
wire [31:0] sw_a6  = sub_word_f(rot_word_f(w[47])) ^ 32'h20000000;  // phase 11
wire [31:0] sw_a7  = sub_word_f(rot_word_f(w[55])) ^ 32'h40000000;  // phase 13

// Even-phase SubWord wires: SubWord(w[k]) -- AES-256 extra SubWord, no RotWord
wire [31:0] sw_b1  = sub_word_f(w[11]);   // phase 2
wire [31:0] sw_b2  = sub_word_f(w[19]);   // phase 4
wire [31:0] sw_b3  = sub_word_f(w[27]);   // phase 6
wire [31:0] sw_b4  = sub_word_f(w[35]);   // phase 8
wire [31:0] sw_b5  = sub_word_f(w[43]);   // phase 10
wire [31:0] sw_b6  = sub_word_f(w[51]);   // phase 12

// -----------------------------------------------------------------------
// Sequential key expansion  (14 clock cycles from key_load pulse)
// -----------------------------------------------------------------------
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        // Phase 0: latch key_in into w[0..7]
        ...
        // ------------------------------------------------------------------
        // Phase 1: w[8..11] from w[0..7]; sw_a1 uses w[7]
        // w[9]  = w[1] ^ w[0] ^ sw_a1
        // w[10] = w[2] ^ w[1] ^ w[0] ^ sw_a1
        // w[11] = w[3] ^ w[2] ^ w[1] ^ w[0] ^ sw_a1
        // ------------------------------------------------------------------
        5'd1: begin
            w[ 8] <= w[ 0] ^ sw_a1;
            w[ 9] <= w[ 1] ^ w[ 0] ^ sw_a1;
        ...
        // ------------------------------------------------------------------
        // Phase 2: w[12..15] from w[4..7]; sw_b1 uses w[11] (now registered)
        // ------------------------------------------------------------------
        5'd2: begin
            w[12] <= w[ 4] ^ sw_b1;
            w[13] <= w[ 5] ^ w[ 4] ^ sw_b1;
            w[14] <= w[ 6] ^ w[ 5] ^ w[ 4] ^ sw_b1;
            w[15] <= w[ 7] ^ w[ 6] ^ w[ 5] ^ w[ 4] ^ sw_b1;
            phase <= 5'd3;
        end

        // ------------------------------------------------------------------
        // Phase 3: w[16..19] from w[8..11]; sw_a2 uses w[15]
        // ------------------------------------------------------------------
        5'd3: begin
            w[16] <= w[ 8] ^ sw_a2;
            w[17] <= w[ 9] ^ w[ 8] ^ sw_a2;
            w[18] <= w[10] ^ w[ 9] ^ w[ 8] ^ sw_a2;
            w[19] <= w[11] ^ w[10] ^ w[ 9] ^ w[ 8] ^ sw_a2;
            phase <= 5'd4;
        end

        // ------------------------------------------------------------------
        // Phase 4: w[20..23] from w[12..15]; sw_b2 uses w[19]
        // ------------------------------------------------------------------
        5'd4: begin
            w[20] <= w[12] ^ sw_b2;
            w[21] <= w[13] ^ w[12] ^ sw_b2;
            w[22] <= w[14] ^ w[13] ^ w[12] ^ sw_b2;
            w[23] <= w[15] ^ w[14] ^ w[13] ^ w[12] ^ sw_b2;
            phase <= 5'd5;
        end

        // ------------------------------------------------------------------
        // Phase 5: w[24..27] from w[16..19]; sw_a3 uses w[23]
        // ------------------------------------------------------------------
        5'd5: begin
            w[24] <= w[16] ^ sw_a3;
            w[25] <= w[17] ^ w[16] ^ sw_a3;
            w[26] <= w[18] ^ w[17] ^ w[16] ^ sw_a3;
            w[27] <= w[19] ^ w[18] ^ w[17] ^ w[16] ^ sw_a3;
            phase <= 5'd6;
        end

        // ------------------------------------------------------------------
        // Phase 6: w[28..31] from w[20..23]; sw_b3 uses w[27]
        // ------------------------------------------------------------------
        5'd6: begin
            w[28] <= w[20] ^ sw_b3;
            w[29] <= w[21] ^ w[20] ^ sw_b3;
            w[30] <= w[22] ^ w[21] ^ w[20] ^ sw_b3;
            w[31] <= w[23] ^ w[22] ^ w[21] ^ w[20] ^ sw_b3;
            phase <= 5'd7;
        end

        // ------------------------------------------------------------------
        // Phase 7: w[32..35] from w[24..27]; sw_a4 uses w[31]
        // ------------------------------------------------------------------
        5'd7: begin
            w[32] <= w[24] ^ sw_a4;
            w[33] <= w[25] ^ w[24] ^ sw_a4;
            w[34] <= w[26] ^ w[25] ^ w[24] ^ sw_a4;
            w[35] <= w[27] ^ w[26] ^ w[25] ^ w[24] ^ sw_a4;
            phase <= 5'd8;
        end

        // ------------------------------------------------------------------
        // Phase 8: w[36..39] from w[28..31]; sw_b4 uses w[35]
        // ------------------------------------------------------------------
        5'd8: begin
            w[36] <= w[28] ^ sw_b4;
            w[37] <= w[29] ^ w[28] ^ sw_b4;
            w[38] <= w[30] ^ w[29] ^ w[28] ^ sw_b4;
            w[39] <= w[31] ^ w[30] ^ w[29] ^ w[28] ^ sw_b4;
            phase <= 5'd9;
        end

        // ------------------------------------------------------------------
        // Phase 9: w[40..43] from w[32..35]; sw_a5 uses w[39]
        // ------------------------------------------------------------------
        5'd9: begin
            w[40] <= w[32] ^ sw_a5;
            w[41] <= w[33] ^ w[32] ^ sw_a5;
            w[42] <= w[34] ^ w[33] ^ w[32] ^ sw_a5;
            w[43] <= w[35] ^ w[34] ^ w[33] ^ w[32] ^ sw_a5;
            phase <= 5'd10;
        end

        // ------------------------------------------------------------------
        // Phase 10: w[44..47] from w[36..39]; sw_b5 uses w[43]
        // ------------------------------------------------------------------
        5'd10: begin
            w[44] <= w[36] ^ sw_b5;
            w[45] <= w[37] ^ w[36] ^ sw_b5;
            w[46] <= w[38] ^ w[37] ^ w[36] ^ sw_b5;
            w[47] <= w[39] ^ w[38] ^ w[37] ^ w[36] ^ sw_b5;
            phase <= 5'd11;
        end

        // ------------------------------------------------------------------
        // Phase 11: w[48..51] from w[40..43]; sw_a6 uses w[47]
        // ------------------------------------------------------------------
        5'd11: begin
            w[48] <= w[40] ^ sw_a6;
            w[49] <= w[41] ^ w[40] ^ sw_a6;
            w[50] <= w[42] ^ w[41] ^ w[40] ^ sw_a6;
            w[51] <= w[43] ^ w[42] ^ w[41] ^ w[40] ^ sw_a6;
            phase <= 5'd12;
        end

        // ------------------------------------------------------------------
        // Phase 12: w[52..55] from w[44..47]; sw_b6 uses w[51]
        // ------------------------------------------------------------------
        5'd12: begin
            w[52] <= w[44] ^ sw_b6;
            w[53] <= w[45] ^ w[44] ^ sw_b6;
            w[54] <= w[46] ^ w[45] ^ w[44] ^ sw_b6;
            w[55] <= w[47] ^ w[46] ^ w[45] ^ w[44] ^ sw_b6;
            phase <= 5'd13;
        end

        // ------------------------------------------------------------------
        // Phase 13: w[56..59] from w[48..51]; sw_a7 uses w[55]
        // Only 4 words — final expansion step. valid asserted.
        // ------------------------------------------------------------------
        5'd13: begin
            w[56] <= w[48] ^ sw_a7;
            w[57] <= w[49] ^ w[48] ^ sw_a7;
            w[58] <= w[50] ^ w[49] ^ w[48] ^ sw_a7;
            w[59] <= w[51] ^ w[50] ^ w[49] ^ w[48] ^ sw_a7;
            valid <= 1'b1;
            phase <= 5'd14;  // idle
        end

        default: ;  // phase = 14: idle, keep valid high until next key_load
...
// -----------------------------------------------------------------------
// Output assembly — same bit layout as aes_key_expand_256:
//   round_keys_flat[1919 - 32*i -: 32] = w[i]
// -----------------------------------------------------------------------
genvar gi;
generate
    for (gi = 0; gi < 60; gi = gi + 1) begin : rk_out
        assign round_keys_flat[1919 - 32*gi -: 32] = w[gi];
    end
endgenerate
