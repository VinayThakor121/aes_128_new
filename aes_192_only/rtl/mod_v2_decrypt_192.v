`timescale 1ns / 1ps
//
// mod_v2_decrypt_192 — timing-optimised two-stage pipelined AES-192 decrypt
//
// Pipeline split
// --------------
//   Stage 1 (S_ROUND_S1): InvShiftRows → InvSubBytes(rk[cnt+1]) → ARK(rk[cnt])
//                          → register ark_pipe
//   Stage 2 (S_ROUND_S2): InvMixColumns(ark_pipe) → state_reg
//
// Critical path per stage
// -----------------------
//   Stage 1: state_reg → InvShiftRows (wiring) → InvSBox (~5.5 ns)
//            → XOR xorkey (~0.5 ns) → XOR rk[cnt] (~0.5 ns) → ark_pipe
//            ≈ 6.5 ns  ✓
//   Stage 2: ark_pipe → InvMixColumns (~3.5 ns) → state_reg  ≈ 4 ns  ✓
//
// Key expansion and latency: same as mod_v2_encrypt_192 (9-cycle key expand,
// total ≈ 35 cycles per operation).  External interface UNCHANGED.
...
//   InvShiftRows → InvSubBytes(rk[cnt+1]) → XOR rk[cnt]
// The rk[cnt+1] adder adds ~0.5 ns but InvSBox (~5.5 ns) dominates,
// so the overall path comfortably fits in 10 ns.
// -----------------------------------------------------------------------
wire [127:0] isr_out;
    ...
    if (rst) begin
        ...
        case (fsm)
            ...
            S_KEY_WAIT: if (rk_valid) begin
                state_reg <= ciphertext ^ get_rk(Nr[3:0]);  // initial ARK with rk[12]
                round_cnt <= Nr[3:0] - 4'd1;                // 11
                fsm       <= S_ROUND_S1;
            end

            // Stage 1: InvShiftRows → InvSubBytes(rk[cnt+1]) → XOR(rk[cnt]) → ark_pipe
            S_ROUND_S1: begin
                ark_pipe <= comb_ark;
